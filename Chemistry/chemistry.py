def main():
    ...
    for i in range (len(table_list)):
        name_output = table_list[i][1]
        mass_output = table_list[i][2]
        print(f"{name_output} {mass_output}")

    """
    user_formula = input("Enter the molecular formula of the sample: ")
    mass_in_grams = input("Enter the mass in grams of the sample: ")
    mass_integer = float(mass_in_grams)
    table_dict = make_periodic_table()
    parsed_formula = parse_formula(user_formula, table_dict)
    molar_mass = compute_molar_mass(parsed_formula, table_dict)
    print(parsed_formula)
    rounded_mass = round(molar_mass, 5)
    print(f"{rounded_mass} grams/mole")
    mole_count = mass_integer / molar_mass
    rounded_moles = round(mole_count, 5)
    print(f"{rounded_moles} moles")


def make_periodic_table():
    """
    Takes no parameters. Returns a compound list.
    """
    periodic_table_dict = {
...
# For each list in the compound symbol_quantity_list:
    table_dict = periodic_table_dict
    total_molar_mass = 0

# Separate the list into symbol and quantity.    
    symbol = [i[0] for i in symbol_quantity_list]
    quantity = [i[1] for i in symbol_quantity_list]

    # Get the atomic mass for the symbol from the dictionary.        
    for i in symbol:
        mini_list = table_dict.get(i)
        atomic_mass = mini_list[1]

# Multiply the atomic mass by the quantity.  
        index = symbol.index(i)
        total_symbol_mass = atomic_mass * quantity[index]

# Add the product into the total molar mass.        
        total_molar_mass = total_molar_mass + total_symbol_mass
